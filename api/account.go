...
// createAccountParams, balence should allways start 0
...
func (server *Server) getAccount(ctx *gin.Context) {
	...
	if authPayload.Username != account.Owner {
		err := errors.New("account dosen't belong to authenticated user")
...
func (server *Server) listAccounts(ctx *gin.Context) {
	...
	// offser: nb of accounts that should skip
...
func (server *Server) deleteAccount(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusOK, "Account deleted succesfly")
}
